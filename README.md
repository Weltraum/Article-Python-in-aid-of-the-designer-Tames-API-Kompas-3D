...
Теперь, что бы подключиться к интерфейсу программы нам понадобиться следующий код:
...
После выполнения нашего скрипта, правильнее всего вернуть всё на свои места, т.е. если программа Компас не была запущена, её надо закрыть. Самый простой способ определить, запущен ли процесс, использовать стандартный модуль subprocess:
...
Данная функция проверяет, запущен ли процесс «KOMPAS» [стандартными методами Windows](https://technet.microsoft.com/en-us/library/bb491010.aspx#mainSection).

## Считаем количество листов и их формат:

Тут всё просто: у нашего документа doc7 имеется интерфейс коллекции листов оформления LayoutSheets. Каждый лист обладает свойством формата и кратности.
...
Придти к этим строчкам не просто, примерно так:
...
На самом деле ячейка под №6 для листа с другим оформлением может быть и не масштаб.  Посмотрим, как в программе Компас 3D определяется стили оформления чертежа:

![Style](https://habrastorage.org/files/371/b6d/490/371b6d490efb4ce59876457318c4513d.png)

Поэтому правильнее проверять какому файлу и номеру оформления соответствует лист чертежа. Так же стоит помнить, что иногда в документ вставляется титульный лист! Поэтому придётся усложнить код. Пришлось применить [регулярные выражения](https://habrahabr.ru/post/115825/), т.к. текст в ячейке может являться ссылкой на вид.
...
И остался последний вопрос, как узнать нужный номер ячейки? Для этих целей удобно создать файл чертежа, в котором интересующие нас ячейки будут заполнены, а после прочитать все возможные варианты с помощью следующей функции:
...
Согласно SDK нам всего то надо получить интерфейс TechnicalDemand от IDrawingDocument, а
...
Стоит отметить, что данный код полагается на автоматическую нумерацию технических требований, и если это не так или, например, технические требования набраны с использованием простого инструмента «Текст», то всё намного сложнее.

## Считаем количество размеров на чертеже:

При подсчёте размеров, надо иметь в виду, что необходимо посчитать их на каждом из виде:
...
def parse_design_documents(paths):
    ...
    if not is_run: app7.Quit()                      # Выходим из программы, если до выполнения скрипта она была не запущена
    return table
```
...
Что бы не рисовать в tkintere интерфейс пользователя, предлагаю воспользоваться хорошей программой Excel, куда и выведем результат нашего труда:
...
Если не полениться и правильно подготовить Excel файл, то результат работы нашего скрипта может быть следующим:

![Здесь красивая цветная картинка из Excel](https://habrastorage.org/files/368/ebd/e0b/368ebde0bc76418b91e57ab9e5a1e7b6.png)

На графике изображено участие сотрудников отдела в процентном соотношении при выпуске документации на изделие.
