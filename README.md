# Python на службе конструктора. Укрощаем API Kompas 3D
...
На самом деле ячейка №6 для листа с другим оформлением может содержать не масштаб, а совсем иную информацию.  Посмотрим, как Kompas 3D определяются стили оформления чертежа:
